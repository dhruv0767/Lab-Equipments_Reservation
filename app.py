...
try:
    ...
    reservations_df = load_or_initialize_df('reservations_df',
                                            ['Username', 'Room', 'Equipment', 'Start_Time', 'End_Time'])
    pcr_reservations_df = load_or_initialize_df('pcr_reservations_df',
                                                ['Username', 'Room', 'Equipment', 'Start_Time', 'End_Time'])
    ...
    def check_and_submit_reservation(selected_room, selected_equipment, start_datetime, end_datetime):
        ...
        st.session_state.pcr_reservations_df['Start_Time'] = pd.to_datetime(
            st.session_state.pcr_reservations_df['Start_Time'])
        st.session_state.pcr_reservations_df['End_Time'] = pd.to_datetime(
            st.session_state.pcr_reservations_df['End_Time'])
    ...
    def clear_reservation_data():
        st.session_state.reservations_df = pd.DataFrame(
            columns=['Username', 'Room', 'Equipment', 'Start_Time', 'End_Time'])
        st.session_state.pcr_reservations_df = pd.DataFrame(
            columns=['Username', 'Room', 'Equipment', 'Start_Time', 'End_Time'])
    ...
    if st.session_state["authentication_status"]:
        ...
        with tab1:
            ...
            room_selection = st.selectbox("### Select a Room", list(room_equipment_details.keys()),
                ...
                if details['enabled']:
                    ...
                    if equipment_reservations.empty:
                        ...
                        for _, reservation in equipment_reservations.iterrows():
                            start = max(reservation['Start_Time'], operational_start) if is_pcr_equipment else \
                            reservation[
                                'Start_Time']
